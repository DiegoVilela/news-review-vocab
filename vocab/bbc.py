...
def _remove_links(bs: BeautifulSoup):
    for el in bs(('p', 'h3')):
        if el.find('a', href=True):
            el.decompose()


def _get_unit_from_episode_id(ep_id):
    if ep_id <= 0:
...
def _get_unit_info(unit=None, episode_id=None):
    """Return a UnitInfo object as per the BBC site."""

    if episode_id:
        unit = _get_unit_from_episode_id(episode_id)
...
def _get_episode_content(sections):
    content = []
    for section in sections:
        for line in section.stripped_strings:
            line_text = str(line)
            if _isRelevant(line_text):
                content.append(line)

    return content[1:]


def _parse_episode(content, episode_id: int):
    ...
    ep_content = _get_episode_content(sections)
    ep_content = '\n'.join(ep_content)
...
class NewsReview:
    ...
    def _get_episode(self, episode_id, unit):
        ...
        try:
            ...
            return _parse_episode(response.content, episode_id)
    ...
    def pull(self):
        ...
        if Episode.objects.exists():
            ...
            unit = _get_unit_info(episode_id=from_episode_id).unit
